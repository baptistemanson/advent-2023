...
pub fn pb1() {
    ...
    INPUT.lines().for_each(|l| {
        let (game, colors) = l[5..].split_once(":").unwrap();
        let is_invalid = colors.split([',', ';']).any(|part| {
            let (nb, color) = part[1..].split_once(" ").unwrap();
            let nb = nb.parse::<i32>().unwrap();
            if color == "red" && nb > 12 {
                return true;
            }
            if color == "green" && nb > 13 {
                return true;
            }
            if color == "blue" && nb > 14 {
                return true;
...
pub fn pb2() {
    let mut sum: i32 = 0;
    INPUT.lines().for_each(|l| {
        let (_game, colors) = l[5..].split_once(":").unwrap();
        let maxes = colors.split([',', ';']).fold([0, 0, 0], |mut acc, part| {
            let (nb, color) = part[1..].split_once(" ").unwrap();
            let idx = match color {
                "red" => 0,
                "green" => 1,
            ...
            acc[idx] = cmp::max(acc[idx], nb.parse::<i32>().unwrap());
            acc
        });
...
#[allow(dead_code)]
const INPUT_T: &str = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
#[allow(dead_code)]
const INPUT: &str = "Game 1: 2 blue, 3 red; 3 green, 3 blue, 6 red; 4 blue, 6 red; 2 green, 2 blue, 9 red; 2 red, 4 blue
