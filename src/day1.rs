...
pub fn pb1() {
    ...
    INPUT.lines().for_each(|l| {
        let first = l.chars().find(|c| char::is_digit(*c, 10)).unwrap();
        let potential_last = l.chars().rev().find(|c| char::is_digit(*c, 10));
        let last = match potential_last {
            Some(a) => a,
            None => first,
        };
...
fn translate(a: &str) -> i64 {
    if a.len() == 1 && char::is_digit(a.chars().next().unwrap(), 10) {
        return a.parse::<i64>().unwrap();
    }
    match a {
        "one" => 1,
        "twone" => 1,
...
pub fn pb2() {
    let mut sum: i64 = 0;
    let re =
        Regex::new(r"(1|2|3|4|5|6|7|8|9|twone|eightwo|eighthree|oneight|threeight|fiveight|nineight|sevenine|one|two|three|four|five|six|seven|eight|nine)").unwrap();
    INPUT.lines().for_each(|l| {
        let it: Vec<&str> = re.find_iter(l).map(|m| m.as_str()).collect();
        let (first, last) = match it.len() {
            1 => (it[0], it[0]),
            _ => (it[0], it[it.len() - 1]),
        };
        let curr = translate(first) * 10 + translate(last);
        sum += curr;
    });
    dbg!(sum);
}
